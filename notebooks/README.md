...
`specdiff/verifiers/dgrs.py`. Alongside them the notebooks use rules that are exact and
...
Speedups attributed to `rmc` and `d-grs` are measured, not projected.
[`verifier_tutorial.ipynb`](verifier_tutorial.ipynb) §10 walks through both rules and the
